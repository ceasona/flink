...
def from_kafka_to_kafka_demo():
    ...
    # query
    transformed_tab = st_env.from_path("source").select('lon, lat')
    table_result = transformed_tab.execute()
    with table_result.collect() as results:
        for result in results:
            print(result)

    transformed_tab.insert_into("mySink")

    # execute
    st_env.execute("2-from_kafka_to_file")
...
def register_rides_sink(st_env):
    st_env.execute_sql("""
             CREATE TABLE mySink (
               ...
               'format' = 'csv',
               'path' = '/home/flink_test/examples2/table/output/kafka2file'
             )
         """)
